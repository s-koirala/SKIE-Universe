...
2. `kpi-report-emitted` → `ninjascript-implemented`: per ADR-0013 §5, mandatory transition in nominal flow; per the user's 2026-05-04 standing directive, this transition is **operator-discretionary upon review of v1 KPI report card values**, NOT auto-mandated. Bridge-mediated implementation per ADR-0013 §1.2 + ADR-0002 if undertaken (HMM forward filter at session-open requires Python inference at decision time per ADR-0005). Tracked under `P1-H052A-NINJASCRIPT-IMPL`.
