...
- **Test statistic** (load-bearing): `T_H062_basket = MPPM(ρ=1)[r_basket_OOS_session_aggregated]` where `r_basket_OOS_session_aggregated` is the **equal-weighted basket per-session aggregated log-return series** across the 4 instruments on the OOS test fold. Per-session aggregation: for each session s and instrument i, `r_{i,s} = Σ_{trade t in session s} log(1 + R_{i,t} × per_trade_dollar_risk_t / equity_t)` (per-trade log-return contributions summed at the per-session level); basket per-session return `r_{basket,s} = (1/4) Σ_i r_{i,s}`. **MPPM(ρ=1) computation uses Δt = 1/252 (session-cadence)** per Goetzmann-Ingersoll-Spiegel-Welch 2007 §2 convention. **R1 F1-005 fix**: prior draft specified per-trade log-return series with Δt=1/252, which is dimensionally inconsistent (per-trade returns are event-driven and irregular-cadence; Δt must match input periodicity per GISW 2007 §2). The corrected per-session aggregation preserves the load-bearing inferential properties of MPPM(ρ=1) while ensuring dimensional consistency with the project-canonical daily Δt convention. **Primary inferential test**: paired stationary-bootstrap CI per [Politis-Romano 1994, *JASA* 89(428):1303-1313](https://doi.org/10.1080/01621459.1994.10476870) with block length per [Politis-White 2004, *Econometric Reviews* 23(1):53-70](https://doi.org/10.1081/ETC-120028836) + [Patton-Politis-White 2009, *Econometric Reviews* 28(4):372-375](https://doi.org/10.1080/07474930802459016) correction. Sharpe-differential family `T_H062_i = SR_breakout_i − SR_passive_BH_i` reported as **secondary KPI** per ADR-0017 §1.2 demotion machinery, under [Ledoit-Wolf 2008 *JEF* 15(5):850-859](https://doi.org/10.1016/j.jempfin.2008.03.002) studentized stationary-bootstrap CI; Hansen 2005 SPA p reported as KPI annotation per [ADR-0008](../../../docs/decisions/ADR-0008-spa-omega-method.md) at M=4 family size (per-symbol).
...
| `P1-ADR-0017-KILL-SWITCH-BACKTEST-VALIDATION` | **CLOSED 2026-05-15 (Phase O.2)** | Wires K-1, K-3, K-4, K-6, K-7 into a post-simulation validator. Module at [src/skie_ninja/backtest/kill_switch_validation.py](../../../src/skie_ninja/backtest/kill_switch_validation.py); 16 regression tests at [tests/unit/test_kill_switch_validation.py](../../../tests/unit/test_kill_switch_validation.py). K-2 enforced structurally by EOD-flatten; K-5 N/A at v1 (no correlated-pair instruments); K-8 enforced at the H062FeatureConfig trend-filter gate. |
| `P1-ADR-0017-DESIGN-MD-CASCADE` | OPEN (not H062-blocking) | BLOCKING-BEFORE-NEXT-STAGE-3-RUN at the project level; project-wide §10 reframing across H050/H051/H052a/H052b/H053/H054. NOT a hard H062 launch blocker per CLAUDE.md Phase O.1 follow-on scope analysis — H062 §10 already uses ADR-0017 framing. |
| `P1-ADR-0018-DESIGN-MD-CASCADE` | OPEN (not H062-blocking) | BLOCKING-BEFORE-NEXT-STAGE-3-RUN at the project level; project-wide MPPM(ρ=1) cascade. NOT a hard H062 launch blocker — H062 design.md §1 already pre-registers MPPM(ρ=1) as primary inference. |
| `P1-CAUSAL-DAG-DESIGN-MD-TEMPLATE` | OPEN (not H062-blocking) | BLOCKING-BEFORE-NEXT-NEW-PRE-REG at the project level; §1.3 stub per ADR-0022. NOT a hard H062 launch blocker — H062 design.md §1.3 already carries the annotation. |
| `P1-QUANT-PROJECT-RULES-CAUSAL-IMPORT` | OPEN (not H062-blocking) | BLOCKING-BEFORE-NEXT-NEW-PRE-REG at the project level; user-level rules amendment. NOT a hard H062 launch blocker. |
| `P1-METALS-ENERGY-COST-MODEL-IMPL` | OPEN (not H062-blocking) | BLOCKING per ADR-0023 for v2 cost model; v1 is zero-cost research-only per H060 v1 precedent. |
| `P1-H062-FEATURE-FACTORY-IMPL` | **CLOSED 2026-05-15 (Phase O.2 batch-1; commit `8772e01`)** | [src/skie_ninja/features/h062/](../../../src/skie_ninja/features/h062/) (donchian.py + features.py + __init__.py; 35 unit tests). |
| `P1-H062-WALK-FORWARD-ORCHESTRATOR-IMPL` | **CLOSED 2026-05-15 (Phase O.2 batch-2; commit `cc2d8a8`)** | [scripts/run_h062_walk_forward.py](../../../scripts/run_h062_walk_forward.py); smoke-tested end-to-end (run_id `33a47f84eff34a53898a089923915f1f`; 142 folds; 10,442 OOS trades). |
| `P1-H062-PIT-CANARY-INTEGRATION-TEST` | **CLOSED 2026-05-15 (Phase O.2 batch-2)** | [tests/integration/test_h062_pit_canaries.py](../../../tests/integration/test_h062_pit_canaries.py); 5 test classes (Canary A boundary-invariant / B label-horizon / C train-test-leak / D NaN-poison / E full composition). |
| `P1-H062-LEVEL-STATE-FOLD-CONTINUITY` | **CLOSED 2026-05-15 (Phase O.2 batch-1)** | [tests/unit/test_h062_level_state_fold_continuity.py](../../../tests/unit/test_h062_level_state_fold_continuity.py); 18 tests asserting embargo arithmetic (purge=2400min, embargo=2400min, total=4800min=960 bars) + channel bit-equality under fold partition. |
| `P1-H062-POWER-SIMULATION-EXECUTE` | **CLOSED 2026-05-15 (Phase O.2 batch-3)** | [scripts/run_h062_spa_power_simulation.py](../../../scripts/run_h062_spa_power_simulation.py); quick-mode K_max_recommended=50 at σ=0.01, effect=0.005 daily-log-ret, T=500; H062.yaml K_max=500 preserved as conservative upper bound; production-mode binding deferred to first-paper-trade cycle. |
| `P1-H062-CALIBRATION-HOLDOUT-RUN` | **CLOSED 2026-05-15 (Phase O.2 batch-3)** | [scripts/run_h062_calibration_holdout.py](../../../scripts/run_h062_calibration_holdout.py); nested-CV Level-A trend_id Brier + Level-B cell-grid MPPM(ρ=1); results in [artifacts/runs/H062/calibration_*/calibration_sidecar.json](../../../artifacts/runs/H062/). |
| `P1-H062-DATA-QUALITY-DEGRADED-DAYS-CANARY` | **CLOSED 2026-05-15 (Phase O.2 batch-2)** | [tests/unit/test_h062_data_quality_canary.py](../../../tests/unit/test_h062_data_quality_canary.py); 10 tests parametrized over MGC+SIL × 3 Databento BentoWarning days (2017-11-13, 2018-10-21, 2019-01-15) + schema-invariant checks. |
| `P1-H062-SUBSTRATE-INGEST-INTO-WORKTREE` | **CLOSED 2026-05-15 (Phase O.2 batch-1)** | Path C canonical re-ingest via `scripts/ingest.py`; both stages emitted output_frame_sha256=`1247dc7ebd2252be837b545b1163702fd8d7bb20512dd3b206e69ec7a0cfe959` — exact match to §16 binding. |
...
- **2026-05-18 — Hsu-Kuan 2005 erratum addendum (Path A frozen-pre-reg amendment).**
  - Post-merge audit-remediate-loop Round 1 literature-check (`docs/audits/audit_trail_2026-05-18_phase-o-merge-audit.md`) surfaced a critical primary-source misattribution: §1.4 partial-decay framing cites Hsu-Kuan 2005 as evidence that "channel breakouts FAIL on large-cap NASDAQ" — verified primary-source finding is the opposite (NASDAQ Composite is in Hsu-Kuan's SURVIVES-SPA category; the distinction in the paper is **market maturity** young-vs-mature, NOT **market capitalization** small-vs-large).
  - Canonical correction at [`research/01_hypothesis_register/_erratum_hsu_kuan_2005_2026-05-18.md`](../_erratum_hsu_kuan_2005_2026-05-18.md). Original §1.4 frozen content preserved verbatim per ADR-0013 §"Frozen pre-registration amendment" §1-§7 immutability discipline.
  - Implication: NQ universe-justification inverts (NQ tracks NASDAQ Composite = SURVIVES-SPA category; modest positive prior-art expectation, NOT "harder-to-beat" as §1.4 claimed). ES + MGC + SIL universe-justification unchanged. H_1 framing + numerical claims unchanged; only interpretive framing of NQ-specific OOS findings reframed.
  - New non-blocking follow-up: `P1-LITERATURE-CHECK-DIRECTIONAL-FINDING-VERIFY` — extend literature-check audit scope to verify paper-finding-direction matches cite-claim-direction.
