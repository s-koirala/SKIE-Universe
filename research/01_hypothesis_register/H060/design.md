...
created: 2026-05-12
created_by: skoir
description: Pre-registered design doc for hypothesis H060 — cross-futures time-series momentum (TSMOM) on equal-weighted CME basket {ES, NQ, MGC, SIL} under the Phase N survival-constrained / MPPM(ρ=1) / Kelly-grid / BOCD-decay / barbell payoff-shape / causal-mechanism paradigm.
---

# H060 — Cross-futures time-series momentum (TSMOM) on {ES, NQ, MGC, SIL}

> **Canonical TSMOM construction on a 4-asset CME-Globex retail-tier basket.** H060 applies the [Moskowitz, Ooi, Pedersen 2012, *JFE* 104(2):228-250](https://doi.org/10.1016/j.jfineco.2011.11.003) (henceforth "MOP 2012") time-series-momentum construction — 12-month look-back signal × ex-ante volatility scaling × monthly rebalance — to the equal-weighted basket of E-mini S&P 500 (ES), E-mini Nasdaq-100 (NQ), COMEX Micro Gold (MGC), and COMEX Micro Silver (SIL) front-month roll-adjusted continuous contracts. **v1 basket amendment 2026-05-12** (operator-authorized; pre-`status: designed` rectification per ADR-0013 §"Frozen pre-registration amendment" recognising the original §1 draft specified a substrate {CL, GC} that was never operator-authorized for extraction — the operator's actual 2026-05-12 Stage A purchase was {MCL, MGC, SIL} at $44.37 USD with MCL subsequently dropped due to 2021-07-12 contract inception precluding the §2 2015-2019 calibration holdout; the v1 basket reflects the operationally-realised substrate). Full crude oil (CL) deferred to **H061** per `P1-H060-V2-WITH-CL-FULL-SIZE` follow-up.
>
...
- **Test statistic** (load-bearing): `T_H060_basket = SR_TSMOM − SR_passive_EW` evaluated under [Ledoit & Wolf 2008, *JEF* 15(5):850-859](https://doi.org/10.1016/j.jempfin.2008.03.002) studentized stationary-bootstrap CI with block length per [Politis & White 2004, *Econometric Reviews* 23(1):53-70](https://doi.org/10.1081/ETC-120028836) + [Patton, Politis & White 2009, *Econometric Reviews* 28(4):372-375](https://doi.org/10.1080/07474930802459016) correction; per-asset SR differentials (`SR_TSMOM_i − SR_passive_i`) reported as sibling family entries under Benjamini-Hochberg FDR at α=0.05 per the project's standing convention. Hansen 2005 SPA p reported as KPI annotation per [ADR-0008](../../../docs/decisions/ADR-0008-spa-omega-method.md) (KPI-only at M=4 family size; not load-bearing for H_1).
...
The four assets {ES, NQ, MGC, SIL} span equity-index (ES, NQ) and metals (MGC, SIL) regimes. Cross-asset correlation is empirically time-varying: equity-commodity correlation flipped from ~-0.2 (1990s-2000s) to +0.3-0.5 (post-2008 QE era) to ~0 (2022-2024 inflation cycle). The TSMOM construction is per-asset and the basket return is the equal-weighted sum — there is NO explicit cross-asset weighting at the signal layer. This is canonical MOP 2012 §2.1 construction; cross-asset weighting variants (risk-parity, equal-vol-contribution) are deferred to a successor hypothesis per `P1-H060-RISK-PARITY-WEIGHTING-SUCCESSOR`.

## 2. Universe and sample period

- **Instruments at v1**: ES (E-mini S&P 500; CME), NQ (E-mini Nasdaq-100; CME), MGC (Micro Gold; COMEX-on-CME-Globex), SIL (Micro Silver; COMEX-on-CME-Globex). All front-month roll-adjusted continuous contracts.  <!-- justify: 4-asset basket is the minimum-diversification cross-asset TSMOM construction per MOP 2012 §2.1 (their basket was 58 instruments; this 4-asset retail-tier basket is the smallest credible projection that preserves equity vs commodity asset-class breadth at the ADR-0001 retail-capacity ceiling). The v1 metals pair (MGC + SIL) is the micro-class analog of the original {CL, GC} draft; operator authorization 2026-05-12 selected the micro-class for retail-tier capacity-match per ADR-0001 and substituted SIL for the second metal to add precious-metals breadth beyond gold-only -->
- **Crude oil deferred to H061**: full-size NYMEX WTI (CL) provides 2015-onward substrate (pre-Micro-WTI-launch); H060 v1 omits crude entirely (MCL inception 2021-07-12 precludes §2 calibration-holdout; full CL extraction at ~$240 USD exceeded operator $80 budget ceiling). H061 reserves the v2 hypothesis ID for "TSMOM with full CL added" per `P1-H060-V2-WITH-CL-FULL-SIZE` follow-up.
- **Micros (MES/MNQ) NOT included at v1**: MES/MNQ are deterministic linear rescalings of ES/NQ per [ADR-0001](../../../docs/decisions/ADR-0001-project-scope.md) and are NOT entered as independent family members; the cost-equivalent micro-substitution is a sizing-layer detail (§4 capacity cap), not a signal-layer decision.  <!-- justify: TSMOM signal is per-asset (sign of past 12mo return); micros share the signal with their major counterparts; treating them as independent family members would inflate the SPA family size at no informational gain -->
- **Substrate availability** (post-Stage-B; 2026-05-12 commit `75f869e`):
  - **Combined substrate SHA256**: `242aaa280b216f45edc3b9d9de9630f52f71206eea7832c1cb0470296190f46f` (38 partitions across ES + NQ + MGC + SIL + MCL in the roll-adjusted layer; MCL data present but NOT used in H060 v1 per the 2026-05-12 operator decision; supersedes the prior ES/NQ-only `b3ee230a...` binding).
  - All four v1 symbols live at [data/processed/vendor_legacy_1min_roll_adjusted/](../../../data/processed/vendor_legacy_1min_roll_adjusted/): ES (2020-2025; 2,016,269 rows), NQ (2020-2024; 1,687,090 rows), MGC (2015-2025; 3,086,887 rows), SIL (2015-2025; 1,868,034 rows).
- **Sample window** (BINDING):
  - **Calibration holdout**: 2015-01-01 → 2019-12-31 (5 years). MGC + SIL contribute the full 5-year window; ES + NQ have no pre-2020 substrate so they contribute zero rows to the calibration holdout. Calibration-holdout role is supplemental at v1 (cross-hypothesis fit-set isolation per H055 precedent); the actual inner-CV operates on IS only.  <!-- justify: disjoint from the test folds of H050/H052a/H053/H054/H055; matches the H055 calibration-holdout convention so cross-hypothesis ID-selection fit-set isolation property is preserved on the metals leg; the asymmetric ES/NQ coverage is documented and accepted -->
  - **IS** (inner-CV grid search across §5 hyperparameter cells): 2020-01-01 → 2023-12-31 (4 years; matches H050 train + H055 IS window). All 4 v1 symbols have data covering the full IS window.
  - **OOS test**: per-symbol substrate right-edges: ES → 2024-01-01 → 2025-12-03; NQ → 2024-01-01 → 2024-12-19; MGC + SIL → 2024-01-01 → 2025-12-30. Per-asset basket-return is computed on the intersection of available symbols at each session (typical ~240-490 trading days of overlap per year).
- **Cadence**: **daily session-close bars** are the canonical TSMOM horizon (MOP 2012 §3 monthly rebalance evaluated on daily closes; Hurst-Ooi-Pedersen 2017 §2 monthly rebalance from daily series). Bar-cadence (1-min) is OUT OF SCOPE for v1 and explicitly NOT a robustness exhibit: bar-cadence trend-following on equity-index futures was demonstrated by H050 KPI report card v1 to produce realised -81%/-84% on the gated arms over the 2024-2025 OOS — bar-cadence is a structurally different regime, not a refinement of TSMOM. Bar-cadence variant is deferred to a separate hypothesis ID per `P1-H060-BAR-CADENCE-OUT-OF-SCOPE`.
  - Daily close definitions: per-instrument's own daily settlement convention. ES + NQ at 16:00 ET RTH-close (the project's standing equity-index session-policy convention); MGC at 13:30 ET COMEX pit-close convention; SIL at 13:25 ET COMEX silver pit-close convention. Per-asset close-time differences make a strict "synchronous daily close" formally impossible — TSMOM signal is computed on each asset's own daily close; the rebalance event is end-of-CME-Globex-trading-day (16:00 CT) on the rebalance day. Logged via `classify_energy_metals_session` in [src/skie_ninja/utils/clock.py](../../../src/skie_ninja/utils/clock.py) per the 2026-05-12 `P1-SESSION-POLICY-24-5-IMPL` closure.
- **Roll treatment**: per-asset roll-adjusted front-month continuous series per the [data/processed/vendor_legacy_1min_roll_adjusted/](../../../data/processed/vendor_legacy_1min_roll_adjusted/) v0.4.0 deliverable; v0.4.0 extension (commit `75f869e`) parameterizes the roll-codes per instrument via `config/instruments.yaml` `roll_rule.codes`, supporting both equity-index quarterly H/M/U/Z (ES, NQ) and monthly F/G/H/J/K/M/N/Q/U/V/X/Z (MGC, SIL). The decade-disambiguation invariant is bypassed for non-quarterly roll-code sets since monthly contracts span calendar-year boundaries normally (e.g., SILZ4 trades late-2023 through Dec-2024); `contract_id_full` uniqueness check still catches true decade collisions downstream.

- **Cost model — ZERO at v1 (operator 2026-05-12 decision; research-only pre-cost)**: Zero commissions, zero exchange fees, zero NFA fees, zero slippage applied at v1. Realized OOS equity is therefore the **pre-cost upper bound**; live trading + paper-trading P/L will both be strictly less than the v1 KPI report card. Cost-realism deferred to `P1-H060-COST-EMPIRICAL-CALIBRATION` (BLOCKING-BEFORE-PAPER-TRADE-EVALUATED-STAGE-TRANSITION per ADR-0017 §3 cost-modelling-realism). Methodological-correctness annotation `cost-zero-v1-pre-cost-research-only` will be carried in every KPI report card emitted at v1.
...
- **K-5 Correlated-instrument inventory cap**: ES+MES inventory shares budget; NQ+MNQ inventory shares budget; GC+MGC inventory shares budget; SI+SIL inventory shares budget. (Cross-asset cap deferred to a successor hypothesis; the v1 4-asset basket is treated as 4 independent inventory budgets at the cap layer.)
...
- **Sub-basket {MGC, SIL}**: metals-pair-only sub-basket. High-breadth potential; low MGC-SIL correlation.
...
- **2026-05-12 (evening) — pre-`run` rectification of v1 basket scope; §1 + §2 amended.**
  - **Trigger**: operator-authorized Databento Stage A extraction 2026-05-12 morning purchased substrate for {MCL, MGC, SIL} (NOT the {CL, GC} the original §1 draft specified — the original draft was authored by an agent before the operator's substrate-purchase decisions crystallized in chat). Operator 2026-05-12 evening decision: drop MCL (2021-07-12 contract inception precludes the §2 calibration holdout 2015-2019); defer full CL to H061. v1 basket settles at {ES, NQ, MGC, SIL}.
  - **Amendment scope**: §1 H_0 / H_1 basket label updated from {ES, NQ, CL, GC} to {ES, NQ, MGC, SIL}; §1.4 partial-decay caveat retained; §2 instruments + substrate-binding updated to reflect the actual post-Stage-B substrate (combined SHA256 `242aaa280b216f45edc3b9d9de9630f52f71206eea7832c1cb0470296190f46f`); §2 cost-model line added per the 2026-05-12 operator zero-cost v1 decision; §11.1 capacity caps updated (ES ≤ 20, NQ ≤ 40, MGC ≤ 5, SIL ≤ 5 per ADR-0001 + ADR-0023 retail-tier).
  - **Framing**: this is a *pre-`run`* amendment per ADR-0013 §"Frozen pre-registration amendment" — the design.md was committed at `status: designed` in commit `0453cad` but never run against substrate, and the §1 basket label specified a substrate ({CL, GC}) that was never operator-authorized for extraction. The amendment rectifies §1 to match the operationally-realised substrate before §11.1's BLOCKING-BEFORE-PRODUCTION-RUN preconditions clear. Operator review at the next stage transition will treat this as a load-bearing pre-emission record.
  - **Cost-zero v1 decision**: per operator 2026-05-12 "drop fees and commissions for now" — H060 v1 emits as a pre-cost research-only artifact; KPI report card carries `cost-zero-v1-pre-cost-research-only` annotation; realized OOS is the upper bound on what the strategy can produce live. Track `P1-H060-COST-EMPIRICAL-CALIBRATION` for v2.
  - **CL deferral**: full-size NYMEX WTI Light Sweet Crude reserved for **H061** with substrate-extraction cost ~$240 USD per the 2026-05-12 cost-dossier. New follow-up `P1-H060-V2-WITH-CL-FULL-SIZE` tracks the H061 pre-registration cycle.
