"""H052a walk-forward orchestrator (Stage-3).

Per H052a frozen pre-reg [research/01_hypothesis_register/H052a/design.md](
../research/01_hypothesis_register/H052a/design.md) + §15 + §15.1 errata
addendum (2026-05-04).
...
8. Compute T_H052a = SR_gated − SR_unconditional on test fold; LW2008
   differential CI + Hansen SPA (m=1 degenerate per ADR-0008).
...
_log = logging.getLogger("h052a_walk_forward")


_PROJECTION_N_PATHS: int = 5_000
_PROJECTION_N_SESSIONS: int = 252  # per ADR-0013 §3.1
_RNG_SEED_DEFAULT: int = 20260423  # design.md §11 line 158
...
def _process_symbol(
    *,
    ...
    # F-Q-6 fix (Round-2 2026-05-04 major): nonstress_state semantics. Per
    # design.md §5 line 81 "ascending by emission-variance of the front-month
    # log-return". The HMM emission features include `realized_vol` (= σ of
    # log-returns; the variance-of-log-returns proxy per design.md §3 line
    # 53 "Realized variance of log-returns ... per Andersen-Bollerslev 1998").
    # Ranking states by their `realized_vol` emission MEAN is equivalent to
    # ranking by emission-variance of log-returns (since realized_vol IS the
    # σ of log-returns; monotonic transform). Lowest-emission-mean state of
    # `realized_vol` = lowest variance state = non-stress = state 0.
    means = hmm.params_.means
    rv_col_idx = feature_cols.index("realized_vol")
    nonstress_state = int(np.argmin(means[:, rv_col_idx]))
    _log.info(
        "[%s] HMM nonstress_state=%d (lowest realized_vol emission mean per "
        "design.md §5 line 81 ascending-by-variance ordering)",
        symbol, nonstress_state,
    )

    # F-Q-4 fix (Round-2 2026-05-04 critical): apply ADR-0005-mandated causal
    # warm-start forward filter `filter_states_from_prior` to the test fold,
    # propagating the train-terminal posterior. The prior `filter_states`
    # call cold-started on the test block, discarding the train-fold sufficient
    # statistic and introducing O(dwell-time) warm-up bias.
    X_test = X_full[test_idx]
    log_alpha_prior = hmm.terminal_log_alpha(X_train)
    # Sessions between train-terminal and test-first per design.md §6 purge=1
    # (one trading session). At session cadence, the gap is bounded by purge +
    # embargo = 2 sessions per AFML §7.4 + §7.4.2.
    ...
    gate_test = (state_test == nonstress_state).astype(np.int64)
    ...
    gated_returns = _per_session_strategy_returns(
        pnl_log=test_pnl, gate=gate_test, cost_log_drag=test_cost,
    )

    # Sharpes.
    sr_uncond = _annualised_sharpe(uncond_returns)
    sr_gated = _annualised_sharpe(gated_returns)
    t_h052a = sr_gated - sr_uncond

    # F-Q-2 fix (Round-2 2026-05-04 critical): LW2008 differential CI API
    # signature `(returns_a, returns_b, *, alpha, n_bootstrap, block_length,
    # bandwidth, bandwidth_strategy, rng)`. Prior `rng_seed=` was a fabricated
    # kwarg that silently TypeError'd, swallowed by the broad except.
    rng_lw = np.random.default_rng(int(rng_seed) + _LW2008_RNG_OFFSET)
    try:
        lw_ci = ledoit_wolf_2008_differential_ci(
            returns_a=gated_returns,
    ...
    # F-Q-1 fix (Round-2 2026-05-04 critical): Hansen SPA API signature
    # `hansen_spa_test(d, *, n_bootstrap, block_length, variant, omega_method,
    # rng)`. Prior `returns_strategies=`/`returns_benchmark=`/`alpha=` were
    # fabricated kwargs. The relative-performance matrix `d` is built as
    # `(gated - uncond)` per H053 v4 reference at scripts/run_h053_stage3_v4.py.
    rng_spa = np.random.default_rng(int(rng_seed) + _SPA_RNG_OFFSET)
    d_matrix = (gated_returns - uncond_returns).reshape(-1, 1)
    try:
    ...
    eq_gated, eq_gated_end, mdd_gated = _equity_curve(gated_returns)
    ...
    w_g, l_g, z_g = _wlz(gated_returns)
    ...
    proj_gated = _bootstrap_forward_projection(
        gated_returns,
        n_paths=_PROJECTION_N_PATHS,
        n_sessions=_PROJECTION_N_SESSIONS,
        rng_seed=int(rng_seed) + _BOOTSTRAP_RNG_OFFSET,
    ...
    return {
        ...
        "performance": {
            ...
            "annualised_sharpe_gated": float(sr_gated) if np.isfinite(sr_gated) else None,
            "t_h052a_annualised": float(t_h052a) if np.isfinite(t_h052a) else None,
        ...
        "realized_oos": {
            ...
            "gated": {
                "ending_equity": eq_gated_end,
                "ending_pct_change": (eq_gated_end / 10_000.0 - 1.0) * 100,
                "max_drawdown_pct": mdd_gated * 100,
                "n_winning_sessions": w_g,
                "n_losing_sessions": l_g,
                "n_zero_sessions": z_g,
            },
        },
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="H052a walk-forward orchestrator")
    parser.add_argument(
        "--config", default="config/hypotheses/H052a.yaml", help="H052a YAML config path"
    )
    ...
    parser.add_argument(
        "--output-dir", default=None, help="Override output dir (artifacts/runs/H052a/<run_id>/)"
    )
...
if __name__ == "__main__":
    ...
    if _missing_pinning:
        raise RuntimeError(
            ...
            "(closes follow-up P1-H052A-BLAS-PIN-CARRY-FORWARD)."
        )
    try:
