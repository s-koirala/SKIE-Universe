...
| `P1-LGB-INNER-CV-RESULT-CHECKPOINT` **[BLOCKING-BEFORE-NEXT-H050-LAUNCH]** | Class B (inner-CV-LGB results lost on cfg-level crash) | **Promoted to blocking 2026-04-29** per [audit_trail_2026-04-29_h050-prod-run-attempt2-os-reboot-bypass.md](../audits/audit_trail_2026-04-29_h050-prod-run-attempt2-os-reboot-bypass.md). The 2026-04-29 19:25 attempt was killed by an OS-initiated reboot at +49 min (well inside any sane per-attempt cap), losing the in-flight cfg's inner-CV-LGB work even with cfg-checkpoint + per-attempt cap raised to 6 hr. Within-cfg / per-inner-fold / per-LGB-draw checkpointing is the load-bearing defense for any external kill (OS reboot, BSOD, hardware) that lands mid-cfg. |
...
| `P1-WAKE-LOCK-BYPASS-INVESTIGATION` **[BLOCKING-BEFORE-NEXT-H050-LAUNCH]** | Class A (regression of ADR-0010 Layer 1) | **New 2026-04-29.** 2026-04-29 19:25 attempt acquired wake-lock `ES_CONTINUOUS \| ES_SYSTEM_REQUIRED` at 19:26:45 yet was killed by Microsoft-Windows-Kernel-Power Event 109 ("Reason: Kernel API") at 20:16:03. Same signature as run-2 (2026-04-27) but with wake-lock active. Five-hypothesis disposition per [audit_trail_2026-04-29_h050-prod-run-attempt2-os-reboot-bypass.md](../audits/audit_trail_2026-04-29_h050-prod-run-attempt2-os-reboot-bypass.md) F-1: H-A (Smart AH dynamic) and H-B (UsoSvc enforcement-deadline) unprobed-after-Round-1; H-D weakly refuted but not fully eliminated; H-C and H-E eliminated by mechanism analysis. Subsumes `P1-PREFLIGHT-SMART-AH-DYNAMIC` from ADR-0010 ledger. **Acceptance criteria** (all required to close): (1) probe `Microsoft-Windows-WindowsUpdateClient/Operational` channel + USO trace channel for the 20:14-20:16 window (addresses H-B); (2) probe Smart Active Hours behaviour on a test rig with manual AH 8/1 set, simulating idle for 90+ min, recording whether `SmartActiveHoursState` materialises (addresses H-A); (3) for each of H-A through H-E, decide eliminated / confirmed / open based on (1)+(2); (4) ship a defense layer for whichever hypothesis remains open (e.g. wevtutil-based pre-reboot canary, Group-Policy-based WU pause for the run window, ES_AWAYMODE_REQUIRED with documented Modern-Standby risk acceptance). Investigation must conclude before another multi-hour H050 launch; the wake-lock alone is not a sufficient defense. |
| `P1-PREFLIGHT-SCRIPT-TIMEOUT` **[BLOCKING-BEFORE-NEXT-H050-LAUNCH]** | Class A (gate auditability paper-only when preflight fails) | **New 2026-04-29.** [scripts/preflight/check_windows_update.ps1](../../scripts/preflight/check_windows_update.ps1) timed out at 60 sec for the 19:25 attempt; supervisor proceeded under `--allow-preflight-warn` with no JSON report. Gates 6, 7, 9, 10 of this ADR were not exercised. Raise timeout to 180 sec (or higher; calibrate empirically) and emit partial JSON on timeout rather than total failure. |
