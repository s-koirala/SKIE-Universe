...
**Severity-tier additions to the residual-risk table (informational on ADR-0011 only).** The `**[BLOCKING-BEFORE-NEXT-HXXX-LAUNCH]**` marker introduced in the 2026-04-29 amendment to the §"Residual risk" table is **informational on ADR-0011 itself** — it labels rows whose disposition gates the next real-substrate launch of the named hypothesis. The marker does **not** change the gate count, gate non-waivability matrix, or post-run audit gate spine, and therefore does **not** trigger a `runbook_schema_version` bump (still `production_run_runbook_v1`). The per-hypothesis runbook inherits the gating semantics via gate 14's "Residual-risk gating consultation" sub-bullet (per the runbook template at [research/_templates/production_run_runbook.md](../../research/_templates/production_run_runbook.md)), which requires the operator to confirm at launch time that no `[BLOCKING-BEFORE-NEXT-<HXXX>-LAUNCH]` row remains open for the named hypothesis. The runbook itself does not need a new schema-version-mandated section. Any future amendment that *propagates* gating into the per-hypothesis runbook in a way that changes the gate count, gate non-waivability matrix, or audit-gate spine **does** require a `runbook_schema_version` bump per the existing contract.
...
| `P1-LGB-INNER-CV-RESULT-CHECKPOINT` **[CLOSED-VIA-COMMIT 2026-04-30]** | Class B (inner-CV-LGB results lost on cfg-level crash) | **Closed 2026-04-30**: per-draw checkpoint module at [src/skie_ninja/backtest/lgb_inner_cv_checkpoint.py](../../src/skie_ninja/backtest/lgb_inner_cv_checkpoint.py) (`SCHEMA_VERSION = "lgb_inner_cv_checkpoint_v1_pickle5"`) saves each (cfg_signature, draw_idx) pickle to `<run_dir>/_lgb_inner_cv_checkpoints/<cfg_sig_short>/draw_<NNNN>.pkl` immediately after each LightGBM HP draw completes. Resume via the existing `--resume-cfg-checkpoint` run_id; merge_resume_into_current() combines prior + current run draws, current overrides on conflict. WARN-but-load on cross-HEAD provenance drift; hard-error on schema drift (mirrors cfg_checkpoint contract). 25 new unit tests at [tests/unit/test_lgb_inner_cv_checkpoint.py](../../tests/unit/test_lgb_inner_cv_checkpoint.py). Orchestrator wiring at [scripts/run_walk_forward.py](../../scripts/run_walk_forward.py): module-level `_LGB_INNER_CV_CURRENT_CTX` set by cfg loop just before `_run_symbol_label_cfg` and cleared in `finally`; `_inner_cv_select_hp` reads it and engages per-draw checkpoint. **Promoted to BLOCKING 2026-04-29**, closed 2026-04-30. |
...
| `P1-WAKE-LOCK-BYPASS-INVESTIGATION` **[CLOSED-VIA-COMMIT 2026-04-30]** | Class A (regression of ADR-0010 Layer 1) | **Closed 2026-04-30** with all 4 acceptance criteria executed; final disposition recorded at [audit_trail_2026-04-29_h050-prod-run-attempt2-os-reboot-bypass.md](../audits/audit_trail_2026-04-29_h050-prod-run-attempt2-os-reboot-bypass.md) §"Disposition after probes (2026-04-30)". (1) `Microsoft-Windows-WindowsUpdateClient/Operational` probe found Event 26 at 20:16:03 — same second as Kernel-Power 109. (2) Smart-AH probe: `SmartActiveHoursState` is unset on this host → H-A eliminated. (3) Final disposition: **H-B (UsoSvc enforcement-deadline) confirmed leading candidate**; H-A/H-C/H-E eliminated; H-D weakly refuted. (4) Defense layer shipped: [scripts/preflight/pause_windows_update.ps1](../../scripts/preflight/pause_windows_update.ps1) sets `PauseUpdatesExpiryTime` for the run window; preflight check reads the pause state and downgrades from "warn (AH gap)" to "ok (WU paused)". Subsumes `P1-PREFLIGHT-SMART-AH-DYNAMIC` from ADR-0010 ledger. **Promoted to BLOCKING 2026-04-29**, closed 2026-04-30. |
| `P1-PREFLIGHT-SCRIPT-TIMEOUT` **[CLOSED-VIA-COMMIT 2026-04-30]** | Class A (gate auditability paper-only when preflight fails) | **Closed 2026-04-30**: `_PREFLIGHT_TIMEOUT_S = 180` in [scripts/supervised_run.py](../../scripts/supervised_run.py) (raised from 60). [scripts/preflight/check_windows_update.ps1](../../scripts/preflight/check_windows_update.ps1) accepts `-OutputPath` and writes incremental JSON after each major check section (init → after_last_boot_time → after_pending_restart → after_active_hours → after_wu_pause → done). On `subprocess.TimeoutExpired`, the supervisor reads the partial JSON from disk and returns it with `status="warn-timeout-partial"` — gates 6/7/9/10 readers can use the partial values written before the timeout. **Promoted to BLOCKING 2026-04-29**, closed 2026-04-30. |
| `P1-RELAUNCH-PER-ATTEMPT-CAP-CALIBRATION` | Threshold derivation | **New 2026-04-29.** `PER_LAUNCH_CAP_S` raised from 10800 (3 hr) to 21600 (6 hr). Empirical evidence supports only "a value > 3 hr is needed" (attempt-1 hit cap with 0 new cfgs); attempt-2 (+49 min) cannot constrain the upper end. **6 hr is a provisional doubling, not an empirical floor.** Calibrate against a per-cfg-completion-time distribution once `P1-LGB-INNER-CV-RESULT-CHECKPOINT` lands per-draw progress markers; calibration must complete before the next H050 launch sequence concludes. |
| `P1-LGB-INNER-CV-CPU-ZERO-INVESTIGATION` | Class B (root-cause clarity for hung cfg) | **New 2026-04-29.** Five consecutive supervisor samples at 30s intervals (20:13:18 → 20:15:18) showed 0% CPU on a single-threaded LGB inner-CV that should be CPU-bound. **Capture stack on next observed 0% CPU event in this priority order**: (a) `py-spy dump --pid <orchestrator>` for Python frame stack; (b) Sysinternals `procdump -ma <pid>` for full minidump (works on apparently-frozen processes; analysable in WinDbg); (c) supervisor-side ETW trace via `wpr -start CPU,FileIO -filemode` for the 60 s preceding any kill. **Auto-trigger**: 5 consecutive 30 s supervisor samples at 0% CPU → auto-procdump (so capture does not depend on operator presence). Soft follow-up; the architectural fix (`P1-LGB-INNER-CV-RESULT-CHECKPOINT`) bounds the cost regardless. |
| `P1-SUPERVISOR-FINALLY-WRITE-ON-HARD-KILL` | Operational | **New 2026-04-29.** Supervisor's terminal `summary.json` was not written for the 19:25 attempt because the OS reboot at 20:16:03 was a hard kill that bypassed the Python `finally` block. Investigate atomic write of partial summary at supervisor process spawn so a hard kill leaves at least the spawn record on disk; current state means audit gate cannot tell whether the supervisor's classification logic ever ran. |
