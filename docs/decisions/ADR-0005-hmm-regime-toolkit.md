...
**Warm-vs-cold filter regression diagnostic.** A passive per-fold diagnostic at [src/skie_ninja/models/regime/diagnostics.py](../../src/skie_ninja/models/regime/diagnostics.py) records the divergence between the production warm-start posterior (`filter_states_from_prior`) and the cold-start regression baseline (`filter_states`) at every walk-forward fold boundary. Primary metric: Hellinger distance ([Le Cam 1986, *Asymptotic Methods in Statistical Decision Theory*, Springer ISBN 978-1-4612-9343-3, §15](https://doi.org/10.1007/978-1-4612-4946-7); [Tsybakov 2009, *Introduction to Nonparametric Estimation*, Springer ISBN 978-0-387-79051-0, §2.4](https://doi.org/10.1007/b13794)) — bounded in [0, 1] under the `H = (1/√2)·||√p − √q||₂` normalisation, proper metric on the simplex, well-behaved under near-deterministic transitions where the symmetric Kullback–Leibler divergence ([Kullback & Leibler 1951, *Ann. Math. Stat.* 22(1):79-86](https://doi.org/10.1214/aoms/1177729694)) diverges. Secondary metric: total-variation distance, with the Tsybakov 2009 Lemma 2.3 envelope `H² ≤ TV ≤ H·√(2 − H²)` asserted as a per-fold sanity flag. Threshold rule: raw values published; no regression-detection threshold in this iteration (no published rule located in the cited sources for HMM warm-vs-cold filter divergence). Sidecar emitted at `logs/reproducibility/{run_id}_warm_cold_diagnostic.json` (`warm_cold_diagnostic_v1` schema); SHA256 combined with the per-fold ledger roll-up into `ReproLog.model_hash` via `with_model_hash`, so a future warm-cold regression flips the run-level hash. Diagnostic is causal (test-fold cold path consumes only test-fold observations; warm path consumes the train-terminal posterior which is itself a function of train observations only) and does not enter the model output, prediction vector, or trading signal. Tracked under follow-up `P1-HMM-WARM-COLD-DIAGNOSTIC`.
...
- [Le Cam 1986, *Asymptotic Methods in Statistical Decision Theory*, Springer-Verlag, ISBN 978-1-4612-9343-3, §15](https://doi.org/10.1007/978-1-4612-4946-7) — Hellinger distance and contiguity (warm-vs-cold filter diagnostic).
- [Tsybakov 2009, *Introduction to Nonparametric Estimation*, Springer, ISBN 978-0-387-79051-0, §2.4](https://doi.org/10.1007/b13794) — distances between distributions, Lemma 2.3 (Hellinger / TV inequalities used in `le_cam_envelope_holds`).
