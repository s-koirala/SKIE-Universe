...
- **F-1-1 + F-1-2:** [scripts/run_walk_forward.py](../../scripts/run_walk_forward.py) `_fit_fold` no longer applies a zero-mask; HMM fits on full `r_tr`. The single construction-zero `r_bar[0] = 0` per symbol contributes <1e-6 of any moment estimate over ~10⁶ training bars and is statistically negligible vs the Hamilton-clock invariant violation that masking introduced. `hmm_train_terminal_position = int(train_idx[-1])` is harvested directly.
