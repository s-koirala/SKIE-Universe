...
- [x] **Cycle 6 — H050 feature factory + first walk-forward run** (2026-04-24) ✓ accepted. 54/54 unit tests green (32 feature + 22 cost model). Audit trail: [docs/audits/audit_trail_2026-04-24_cycle6-h050-feature-factory.md](../docs/audits/audit_trail_2026-04-24_cycle6-h050-feature-factory.md). Three-round parallel triad (quant-auditor + literature-check + reproducibility-verifier). Round-1 cleared 2 criticals (Yang-Zhang k-formula wrong: α=1.34→0.34; OFI not normalized; test expected raw sums not ratios; ADR-0007 Snippet 7.3→7.1; ADR-0008 §2.2→§3; provenance JSON missing `output_frame_sha256`). Round-2 cleared 4 majors (OFI float-NaN guard, `test_no_silent_nan` missing `is_nan().sum()`, `_MIN_OOS_FOR_CI=30`, simultaneous-barrier comment fix, logging.warning on gate skip, YZ docstring TTR cross-check). Round-3 cleared Repro-1 (`output_frame_sha256` computed and written in `emit_provenance`), F-3-2 (magic threshold 10 → `_MIN_VALID_RETURNS_FOR_MASK=2`); F-3-1 (HMM warm-start at fold boundary) and F-3-3 (split params from H050.yaml date boundaries) documented as **blocking before evidence-bar** run with follow-ups `P1-HMM-FOLD-WARM-START` and `P1-H050-SPLIT-PARAMS`. Phase-A composition verified end-to-end in dry-run mode. Key deliverables: [src/skie_ninja/features/](../src/skie_ninja/features/) (labels + 4 microstructure modules), [src/skie_ninja/backtest/costs/nt8_es_nq_rth_v1.py](../src/skie_ninja/backtest/costs/nt8_es_nq_rth_v1.py), [scripts/run_walk_forward.py](../scripts/run_walk_forward.py), [research/01_hypothesis_register/H050/data_requirements.md](../research/01_hypothesis_register/H050/data_requirements.md), [config/hypotheses/H050.yaml](../config/hypotheses/H050.yaml), [docs/decisions/ADR-0007](../docs/decisions/ADR-0007-embargo-placement.md) + [ADR-0008](../docs/decisions/ADR-0008-spa-omega-method.md).
