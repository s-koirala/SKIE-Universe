...
- **Cycle 6 ✓ done (2026-04-24)** — H050 feature factory + walk-forward orchestrator. Yang-Zhang vol + triple-barrier labels (`TripleBarrierLabeler`); 4 microstructure features (Parkinson RV, realized variance, realized skew, OFI tick-rule proxy; all PIT-safe, deterministic, no-silent-NaN); NT8 ES/NQ RTH cost model (`NT8EsNqRthV1CostModel`); Phase-A walk-forward orchestrator (`scripts/run_walk_forward.py`) composing all cycles 1–5 + LightGBM + HMM gating + Opdyke CI + Hansen SPA; `data_requirements.md` frozen checksums; `H050.yaml` config; ADR-0007 (stacked embargo) + ADR-0008 (SPA omega). 54 unit tests green. Audit trail: [docs/audits/audit_trail_2026-04-24_cycle6-h050-feature-factory.md](docs/audits/audit_trail_2026-04-24_cycle6-h050-feature-factory.md). 3-round parallel triad cleared: Yang-Zhang k-formula bug (α=1.34→0.34), OFI NaN guard + volume normalization, `test_no_silent_nan` float-NaN detection, `_MIN_OOS_FOR_CI=30` constant, `output_frame_sha256` written by `emit_provenance`. **Blocking before evidence-bar**: `P1-HMM-FOLD-WARM-START` (HMM warm-start across fold boundaries), `P1-H050-SPLIT-PARAMS` (derive fold sizes from H050.yaml date boundaries, not n//3/n//10). Other follow-ups: `P1-H050-INNER-CV`, `P1-H050-FEATURE-PIT-ASSERT`, `P1-H050-LABEL-CV`, `P1-H050-COST-EMPIRICAL-CALIBRATION`.
